...
Recovery selects and fully validates the highest canonical snapshot when present, then validates canonical segment names and headers for the exact suffix beginning at snapshot watermark plus one. Effects install without command admission and all `SlackTimeline` indexes rebuild once after snapshot state and the final suffix segment are accepted. Without a snapshot, recovery remains unchanged and begins at record one. The only record mutation is truncation and synchronization of an incomplete record in the final active segment. Sealed partial tails and all complete corruption remain fatal.

`EngineSnapshot` is the crate-private authoritative capture/restore boundary. It retains all pools, all promises including terminal states, ordered events, exact idempotency hashes/responses, domain sequence, and runtime publication revision, but no clock or `SlackTimeline`. Snapshot installation creates a durable empty suffix segment before capture, atomically renames a checksummed snapshot, and only then deletes covered WAL segments and older snapshots. This ordering makes cleanup retryable and prevents compaction from removing the only durable recovery path.
